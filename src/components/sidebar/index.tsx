...
import StreamerTooltip from "./tooltip";
...
function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "bg-dark text-white overflow-y-auto scrollbar-hide transition-all",
        ...
        <Accordion type="multiple" defaultValue={StreamerInfos.map(({ line }) => line)} className="mt-4 space-y-1">
          {StreamerInfos.map(({ line, icon, members }) => (
            <AccordionItem key={line} value={line} className="border-b border-white/10">
              ...
              <AccordionTrigger
                className={cn(
                  "flex items-center py-2 hover:bg-white/5",
                  isOpened ? "gap-3 px-2 hover:no-underline" : "justify-center px-2",
                )}
              >
              ...
              {/* 스트리머 리스트 */}
              <AccordionContent>
                <ul className={cn(isOpened ? "px-4 py-1 space-y-[6px]" : "flex flex-col items-center gap-[10px] py-2")}>
                  {members.map((m) => {
